...
// Examples for 2.「合作」
function filter(array: Array<number>, accept: (num: number) => boolean): Array<number> {
...
function map(array: Array<number>, transform: (num: number) => boolean): Array<boolean> {
...
function sort(array: Array<number>, compare: (a: number, b: number) => number): Array<number> {
    if (array.length === 0) {
        return [];
    }
